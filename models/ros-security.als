...
lone sig Attacker extends RosNode {
	...
	// it can only publish stuff that it knows
	all e : this.invokes[Publish] {
		e.msg in VelMsg implies {
			e.msg.velocity in reads.(e.at)
		}
	}
	...
	all t : Time - last | let t' = t.next {
		// cmdsGenerate can only change during callback to CmdVel topic
		reads.t' != reads.t implies
			some e : callee.this & Callback | t = e.at and reads.t' = reads.t + e.m.velocity
	}
}
...
fact InitState {
	...
	no Attacker.reads.first & JoystickNode.operatorBehavior[LaserData]
...
// Every wheel command must have been generated by the trusted operator
pred wheelCommandIntegrity {
	all t : Time, v : Velocity |
		v in WheelNode.cmdsGenerated.t implies
...
pred wheelCommandConfidentiality {
	all t : Time, v : Velocity |
		v in JoystickNode.operatorBehavior[LaserData] implies
			v not in Attacker.reads.t
}

check WheelCommandConfidentiality {
	wheelCommandConfidentiality		
} for 3 but 6 Time, 5 Event 
...
fun _relevant : RosMsg -> Time {
	{rm : RosMsg, t : Time | rm in at.t.(msg + m)}
}

fun _relevant : RosNode -> Time {
	{rn : RosNode, t : Time | rn in at.t.(caller + callee) or t = TO/last}
}
